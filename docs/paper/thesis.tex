...
\begin{figure}
  ...
  \begin{tikzpicture}
    ...
    \addplot[set11,mark=*,mark options={fill=white}, very thick] table[x=X,y expr=\thisrow{$TOTAL}] {benchmarks/cuda_gmres_numerical_2D.csv};
    \addplot[set12,mark=*,mark options={fill=white}, very thick] table[x=X,y expr=\thisrow{$TOTAL}] {benchmarks/cuda_gmres_analytical_2D.csv};
    \addplot[set13,mark=*,mark options={fill=white}, very thick] table[x=X,y expr=\thisrow{$TOTAL}] {benchmarks/openmp_direct_numerical.csv};
    \addplot[set14,mark=*,mark options={fill=white}, very thick] table[x=X,y expr=\thisrow{$TOTAL}] {benchmarks/openmp_direct_analytical.csv};

    \legend{CUDA (GMRES\, Numerical), CUDA (GMRES\, Analytical), OpenMP (Direct\, Numerical), OpenMP (Direct\, Analytical)}
    \end{axis}
    \setmathfont{XITS Math}
  \end{tikzpicture}
  \caption{Total time per timestep using the average over 10 timesteps. First timestep is excluded as warmup. Assuming linear scaling for Fortran.}
...
\begin{figure}
  ...
  \begin{tikzpicture}
    ...
    \legend{Solve System (Fortran\, Original), Solve System (CUDA\, GMRES), Solve System (CUDA\, BiCGStab), Solve System (CUDA\, MAGMA), Solve System (OpenMP\, OpenBLAS)}
    \end{axis}
    \setmathfont{XITS Math}
  \end{tikzpicture}
...
\begin{figure}
  ...
  \begin{tikzpicture}
    ...
    \addplot[color=set13,mark=*,mark options={fill=white}, very thick] table[x=X,y=ASSEMBLE_MATRIX] {benchmarks/openmp_direct_numerical.csv};
    \addplot[color=set14,mark=*,mark options={fill=white}, very thick] table[x=X,y=ASSEMBLE_MATRIX] {benchmarks/openmp_direct_analytical.csv};

    \legend{Assemble System (Fortran\, Analytical), Assemble System (CUDA\, Numerical), Assemble System (OpenMP\, Numerical), Assemble System (OpenMP\, Analytical)}
    \end{axis}
    \setmathfont{XITS Math}
  \end{tikzpicture}
