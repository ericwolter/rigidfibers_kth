...
\begin{tikzpicture}
\setmathfont{FiraSans-Book.otf}
\pgfplotstableread{benchmarks/assemble_system.csv}\loadedtable
\begin{axis}[
...
\addplot[set11,mark=*,mark options={fill=white}, very thick] table[x=X,y=F90Multi_analytic] {\loadedtable};
\addplot[set12,mark=*,mark options={fill=white}, very thick] table[x=X,y=2D] {\loadedtable};
\addplot[set13,mark=*,mark options={fill=white}, very thick] table[x=X,y=1D] {\loadedtable};
\addplot[set14,mark=*,mark options={fill=white}, very thick] table[x=X,y=2D_FP32] {\loadedtable};

\legend{Fortran Multi Analytic, 2D, 1D, 2D Single Precision}
\end{axis}
\setmathfont{XITS Math}
\end{tikzpicture}
