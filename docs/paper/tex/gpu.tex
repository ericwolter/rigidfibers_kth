...
Another option to avoid the slow performance of the global memory is to make sure that the data, which each individual thread requires from global memory, is small. The associated latency when reading data from global memory can be hidden when enough calculations are performed on this data. The GPU is then able to quickly switch to another set of threads and continue the computation while waiting for data from global memory.
