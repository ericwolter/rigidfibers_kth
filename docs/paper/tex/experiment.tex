  ...
  \begin{subfigure}[h]{.48\textwidth}
	  \begin{tikzpicture}
	    \begin{axis}[
	      ...
	      \addplot[color=set11, mark=*,mark options={fill=white}, very thick] table[x=Timestep,y=RadiusZStd,col sep=comma] {charts/sphere_2000_0040_highlight.csv}
	node[pos=444, pin={[pin edge={color=set11, very thick}]min}]{};
...
We observe that the standard deviation of the radius in the direction of gravity in Figure~\ref{fig:torus_deviation} continuously decreases as time passes. The standard deviation then reaches its minimum a few moments before the break up can be identified visually. We can now define this minimum of the standard deviation of the radius as a metric for the break up time. We verified this metric manually against many simulations and it showed great agreement with the visual break up. Using this metric we are now able to investigate the effect of some simulation parameters on the stability and break up time of the torus.

We also looked at the fact that the standard deviation of the radius $R_z$ seems to oscillate in Figure~\ref{fig:torus_deviation}. This oscillation in the direction of gravity means that the torus is periodically expanding and shrinking until it breaks up. This effect can also be seen in the mean sedimentation velocity $V_z$ of the torus in Figure~\ref{fig:torus_velocity}. This periodical change in velocity is similar to the motion observed in the simple tumbling orbits experiment in Section~\ref{sec:example_ring}. This result strongly supports the idea that the turning torus with $2000$ fibers resembles a more chaotic version of the simplified tumbling orbits.
...
The final experiment we performed resembles the setup investigated by Bülow et al.~\cite{Bulow2015}. In this setup instead of identical fibers, we instead look at the mixing of fibers with different densities. We divide the total number of fibers into two different density groups. The two groups are arranged inside a sphere which sediments due to gravity. The two different groups are colored in red and blue. The blue group has the lower density and the red group the higher density. The ratio between the densities is set to $\text{blue} / \text{red}= 0.75 / 1.0$.

Figure~\ref{fig:unmixed_sphere} shows a simulation where the two density groups are separated into the top and bottom half of the sphere. The higher density group is on top and the lower density group is at the bottom (Figure~\ref{fig:mixing_top_a}. After a few timesteps the higher density fibers have almost completely dropped through the lower density fibers (Figure~\ref{fig:mixing_top_b}. The falling high density fibers cause a large number of low density fibers to be ejected from the sphere. Both groups then proceed to almost independently form the characteristic torus observed in the previous experiment in Section~\ref{sec:example_sphere} (Figure~\ref{fig:mixing_top_c}. The higher density fibers, however, form a larger torus with a lower fiber concentration. This lower concentration also causes them to lose velocity, which in turn allows the lower density fibers to drop through the larger outer torus (Figure~\ref{fig:mixing_top_d}. Both the lower and the higher density toruses are not very stable and soon afterwards break down.
...
\begin{figure}[!htbp]
  ...
  \end{subfigure}
  \caption[Unmixed sphere]{Unmixed sphere. Higher density red group is the top half of the sphere. Lower density blue group is the bottom half of the sphere.}
  \label{fig:unmixed_sphere}
\end{figure}

Figure~\ref{fig:mixed_sphere} shows a similar simulation, however, instead of an unmixed sphere, the two groups of fibers are randomly mixed (Figure~\ref{fig:mixing_random_a}). The beginning of the simulation behaves similar to the unmixed case, but the separation does not happen as fast and far fewer lower density fibers are lost from the sphere (Figure~\ref{fig:mixing_random_b}. Both the larger higher density and the smaller lower density torus form again. Yet in contrast to the unmixed case both groups stay closer together. The close proximity prevents the groups from completely separating and causes the higher density torus to be sucked down by the passing lower density torus (Figure~\ref{fig:mixing_random_c}. The two toruses appear to form an interlocked torus, which continues to sediment  (Figure~\ref{fig:mixing_random_d}. Eventually the interlocked torus breaks up and fibers scatter. 
...
\begin{figure}[!htbp]
  ...
  \end{subfigure}
  \caption[Mixed sphere]{Mixed sphere. Higher density red group and lower density blue group are randomly mixed in the sphere.}
  \label{fig:mixed_sphere}
\end{figure}

The results for the mixed and unmixed sphere with rigid straight fibers do largely agree with the results of spherical particles from Bülow et al.~\cite{Bulow2015}. However, both simulation methods and setups are very different which makes a comparison very challenging. Furthermore our experiment are only a first step and many more experiments have to be cared out to get a deeper understanding on the behavior of rigid fibers in a mixed sphere. Exploring the impact of initial fiber configuration, the density ratio and the fiber concentration all are exciting future research opportunities.
